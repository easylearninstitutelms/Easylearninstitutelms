...
let homeworkSchemaReady: Promise<void> | null = null;

export function ensureHomeworkSchema() {
  if (!homeworkSchemaReady) {
    homeworkSchemaReady = (async () => {
      await db.execute(sql`
        ...
        ALTER TABLE homework ADD COLUMN IF NOT EXISTS course_class_id uuid;
        ALTER TABLE homework ADD COLUMN IF NOT EXISTS programme_class_id uuid;
        ALTER TABLE homework ADD COLUMN IF NOT EXISTS teacher_id uuid;
        ALTER TABLE homework ADD COLUMN IF NOT EXISTS title varchar(255);
        ...
        ALTER TABLE homework ADD COLUMN IF NOT EXISTS created_at timestamp NOT NULL DEFAULT now();
        ...
        ALTER TABLE homework_submissions ADD COLUMN IF NOT EXISTS answer text;
        ALTER TABLE homework_submissions ADD COLUMN IF NOT EXISTS attachment_url text;
        ALTER TABLE homework_submissions ADD COLUMN IF NOT EXISTS submitted_at timestamp NOT NULL DEFAULT now();
        ALTER TABLE homework_submissions ADD COLUMN IF NOT EXISTS updated_at timestamp NOT NULL DEFAULT now();

        CREATE UNIQUE INDEX IF NOT EXISTS homework_submissions_homework_student_idx
          ON homework_submissions(homework_id, student_id);

        CREATE INDEX IF NOT EXISTS homework_submissions_institute_idx
          ON homework_submissions(institute_id);

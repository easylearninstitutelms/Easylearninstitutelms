...
interface BatchOption { batch: { id: string; name: string }; studentCount: number }
interface EnrolledStudent { student: { id: string; name: string; studentId: string } | null; enrollment: { id: string } }
interface AttendanceRecord { attendance: { id: string; studentId: string; status: string; date: string }; studentName: string; studentId: string }
...
export default function AttendancePage() {
  ...
  const loadBatchData = useCallback(async () => {
    ...
    try {
      const [batchRes, attRes] = await Promise.all([
        ...
        fetch(`/api/attendance?batchId=${selectedBatch}&date=${selectedDate}`),
      ]);
      ...
      // Pre-fill marks from existing
      const initialMarks: Record<string, string> = {};
      students.forEach(s => {
        if (!s.student) return;
        const found = existingAtt.find(a => a.attendance.studentId === s.student!.id);
        initialMarks[s.student.id] = found ? found.attendance.status : "PRESENT";
      });
  ...
  const StatusBtn = ({ status, current, studentId }: { status: string; current: string; studentId: string }) => {
    ...
    const inactive = "bg-slate-50 text-slate-400 border border-slate-200";
    return (
      <button
        onClick={() => setMarks(prev => ({ ...prev, [studentId]: status }))}
        className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all ${current === status ? active : inactive}`}
      >
    ...
    <div className="space-y-5">
      <div className="page-header">
        <div>
          ...
          <p className="text-sm text-slate-500">Mark daily attendance for batches</p>
        </div>
      </div>
      ...
      {selectedBatch && (
        <>
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              { label: "Present", count: summary.present, color: "bg-green-50 text-green-700" },
              { label: "Absent", count: summary.absent, color: "bg-red-50 text-red-700" },
              { label: "Late", count: summary.late, color: "bg-yellow-50 text-yellow-700" },
              { label: "Leave", count: summary.leave, color: "bg-blue-50 text-blue-700" },
            ].map(s => (
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-slate-700">
                Mark Attendance — {new Date(selectedDate).toLocaleDateString("en-BD", { weekday: "long", month: "long", day: "numeric" })}
              </h2>
                    ...
                    <div key={s.id} className="flex items-center justify-between py-3 border-b border-slate-50 last:border-0">
                      <div className="flex items-center gap-3">
                        ...
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                          currentStatus === "PRESENT" ? "bg-green-100 text-green-600" :
                          currentStatus === "ABSENT" ? "bg-red-100 text-red-600" :
                          currentStatus === "LATE" ? "bg-yellow-100 text-yellow-600" :
                      ...
                      <div className="flex items-center gap-1">
                        <StatusBtn status="PRESENT" current={currentStatus} studentId={s.id} />
                        <StatusBtn status="ABSENT" current={currentStatus} studentId={s.id} />
                        <StatusBtn status="LATE" current={currentStatus} studentId={s.id} />
                        <StatusBtn status="LEAVE" current={currentStatus} studentId={s.id} />
                      </div>
                    </div>
                  );
                })}
      ...
      {!selectedBatch && (
        <div className="card text-center py-16">
          ...
          <p className="text-slate-500 font-medium">Select a batch to take attendance</p>
          <p className="text-sm text-slate-400 mt-1">Choose a batch and date from the controls above</p>
        </div>
      )}
    </div>
  );

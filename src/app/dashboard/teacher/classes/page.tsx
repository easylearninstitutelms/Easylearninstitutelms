...
type ClassRow = {
  ...
  sessionStatus: "PENDING" | "COMPLETED";
  takenAt: string | null;
};
...
export default function TeacherClassesPage() {
  ...
  async function markClass(row: ClassRow, status: "COMPLETED" | "PENDING") {
    ...
    try {
      const res = await fetch("/api/teacher/classes", {
        ...
        body: JSON.stringify({ classId: row.classId, batchId: row.batchId, status }),
      });
      const body = await res.json();
  ...
  if (error) {
    return <div className="card"><p className="text-red-600 text-sm">{error}</p><button className="btn btn-primary mt-4" onClick={load}>Try Again</button></div>;
  }
    ...
    <div className="space-y-6">
      <div className="page-header">
        <div>
          ...
          <p className="text-sm text-slate-500">See which class you have taken and which classes are pending.</p>
        </div>
      ...
      {grouped.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="font-semibold text-slate-700">No active class assigned</p>
          <p className="text-sm text-slate-400 mt-1">A programme, semester and active batch must be assigned to you.</p>
        </div>
      ) : (
        grouped.map((group) => {
          ...
          return (
            <section key={group.key} className="card">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                  <h2 className="text-lg font-bold text-slate-800">{group.programmeName}</h2>
                  <p className="text-sm text-slate-500 mt-1">
                    Batch {group.batchNo ?? "—"} · {group.batchName} · {group.semesterName}
                  </p>
                </div>
              ...
              <div className="mt-5 space-y-3">
                {group.items.map((row) => {
                  ...
                  return (
                    <div key={key} className={"rounded-2xl border p-4 transition " + cardClass(status)}>
                      <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                        <div className="w-14 h-14 rounded-2xl bg-white border border-slate-200 flex flex-col items-center justify-center shrink-0">
                          <span className="text-[10px] uppercase text-slate-400">Class</span>
                          <span className="text-xl font-black text-slate-700">{row.classNo}</span>
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className={"rounded-full px-2.5 py-1 text-xs font-bold " + (
                              ...
                              {status === "COMPLETED" ? "✓ Taken" : status === "TODAY" ? "Today" : "Pending"}
                            </span>
                          </div>
                          {row.description && <p className="text-sm text-slate-500 mt-1">{row.description}</p>}
                          <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-slate-500">
                            <span>📅 {row.scheduledDate || "Date not set"}</span>
                        ...
                        <div className="shrink-0">
                          {row.sessionStatus === "COMPLETED" ? (
                            ...
                            <button
                              disabled={saving === key}
                              onClick={() => markClass(row, "COMPLETED")}
                              className="btn btn-primary"
                            >
                              {saving === key ? "Saving..." : "✓ I Took This Class"}
                            </button>
                          )}
                        </div>
                      </div>

...
export default function HomeworkPage() {
  ...
  const [error, setError] =
    useState("");
  const [selectedHomework, setSelectedHomework] = useState<HwRow | null>(null);
  ...
  async function openSubmissions(row: HwRow) {
    setSelectedHomework(row);
    setSubmissions([]);
    setSubmissionsLoading(true);
    try {
      const res = await fetch(`/api/homework/submissions?homeworkId=${encodeURIComponent(row.id)}`, { cache: "no-store" });
      const data = await res.json();
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        {selectedHomework && (
          <div className="modal-overlay" onClick={(event) => {
            if (event.target === event.currentTarget) setSelectedHomework(null);
          }}>
            <div className="modal-box">
              <div className="modal-header">
                <div>
                  <h2 className="modal-title">Homework Submissions</h2>
                  <p className="mt-1 text-xs text-slate-500">{selectedHomework.title}</p>
                </div>
                <button type="button" onClick={() => setSelectedHomework(null)} className="btn btn-ghost btn-sm">X</button>
              </div>
              <div className="modal-body">
                {submissionsLoading ? (
                  <div className="py-10 text-center text-sm text-slate-500">Loading submissions...</div>
                ) : submissions.length === 0 ? (
                  <div className="py-10 text-center text-sm text-slate-500">No student has submitted this homework yet.</div>
                ) : (
                      ...
                      <div key={item.id} className="rounded-xl border border-slate-200 p-4">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div>
                            ...
                            <p className="text-xs text-slate-400">Student ID: {item.studentCode || "-"}</p>
                          </div>
                          <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">
                            Submitted {item.submittedAt ? formatDate(item.submittedAt) : ""}
                          </span>
                        </div>
                        {item.answer && <div className="mt-3 rounded-lg bg-slate-50 p-3 text-sm text-slate-600 whitespace-pre-line">{item.answer}</div>}
                        {item.attachmentUrl && (
                          <a href={item.attachmentUrl} target="_blank" rel="noreferrer" className="btn btn-outline btn-sm mt-3">

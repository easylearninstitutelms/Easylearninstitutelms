      ...
      const response =
        await fetch("/api/exams", {
          ...
          body: JSON.stringify({
            ...
            programmeClassId:
              examMode === "PROGRAMME" ? examProgrammeClassId : null,
            courseId:
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-purple-200 bg-purple-50/50 px-5 py-4">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="font-semibold text-gray-900">Marksheet</h2>
              <p className="mt-1 text-xs text-gray-600">Select an exam below, enter marks, then use View Marksheet for the student A4 result.</p>
            </div>
            <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-purple-700">A4 Printable</span>
          </div>
        </div>
                    ...
                    <select
                      ...
                      onChange={(event) => {
                        ...
                        if (value === "BATCH") {
                          ...
                          setExamProgrammeClassId("");
                          if (firstProgramme?.id) void loadProgrammeClasses(firstProgramme.id);
                          setExamCourseId("");
                          setExamCourseClassId("");
                          setCourseClasses([]);
                          setExamBatchId("");

...
export default function BatchesPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!r.ok) {
        throw new Error(d?.error || "Failed to load batches");
      }
      ...
      setForm((x) => ({
        ...x,
        batchNo: x.name ? x.batchNo : String(n),
      }));
    } catch (e) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!r.ok) {
        throw new Error(
          d?.error ||
            (editing ? "Failed to update batch" : "Failed to create batch"),
        );
      }
  ...
  async function removeBatch(id: string, name: string) {
    if (
      !confirm(
        `Delete batch "${name}"? This removes linked enrollments, attendance, homework and exam records.`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (!r.ok) {
        throw new Error(d?.error || "Failed to delete batch");
      }
      ...
      <div className="page-header">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Batch numbering starts from 211
          </p>
        </div>
            ...
            <div key={row.id} className="card">
              ...
              <div className="mt-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-slate-500">Programme No</span>
                  <b>{row.programmeNo || "—"}</b>
                </div>
                ...
                <div className="flex justify-between">
                  ...
                  <b className="text-blue-600">{row.studentCount || 0}</b>
                </div>

                <div className="flex justify-between">
                  <span className="text-slate-500">Fee/Month</span>
                  <b>{row.fee ? formatCurrency(row.fee) : "—"}</b>
                </div>
              ...
              <div className="mt-4 flex justify-end gap-2 border-t pt-3">
                <button
                  ...
                  className="btn btn-outline btn-sm text-blue-700"
                >
                  Edit
                </button>

                <button
                  onClick={() => removeBatch(row.id, row.name)}
            ...
            <div className="modal-header">
              <div>
                ...
                <p className="text-xs text-slate-400">
                  {editing
                    ? `Batch #${editing.batchNo || "—"}`
                    : `Next available batch number: ${nextNo}`}
                </p>
              </div>
            ...
            <form onSubmit={submit}>
              <div className="modal-body grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="number"
                    className="form-input font-bold bg-slate-50"
                    value={form.batchNo}
                    readOnly
                  />
                </div>
                ...
                <div>
                  ...
                  >
                    <option value="">
                      {selected ? "Select semester" : "No programme selected"}
                    </option>

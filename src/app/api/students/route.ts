  ...
  try {
    ...
    if (batchId) {
      const [batch] = await db
        .select({ id: batches.id, teacherId: batches.teacherId })
        .from(batches)
      ...
      if (session.role === "TEACHER") {
        const [teacher] = await db
          .select({ id: staff.id })
          .from(staff)
          .where(and(eq(staff.userId, session.userId), eq(staff.instituteId, instituteId)))
          .limit(1);
    ...
    if (session.role === "TEACHER" && !batchId) {
      ...
      conditions.push(sql`
        EXISTS (
          SELECT 1
          FROM ${enrollments}
          INNER JOIN ${batches} ON ${batches.id} = ${enrollments.batchId}
          WHERE ${enrollments.studentId} = ${students.id}
            AND ${enrollments.status} = 'ACTIVE'
...
export async function POST(request: Request) {
  ...
  const permissionError = requireRoles(session, STUDENT_MANAGE_ROLES);
  if (permissionError) return permissionError;
  ...
  try {
    ...
    if (body.photoUrl && !photoUrl) {
      return Response.json(
        { error: "Invalid student photo. Please choose a JPG, PNG, or WEBP image up to 2 MB." },
        { status: 400 },
      );
    }
    ...
    for (const row of ((existingStudentIds as any).rows || [])) {
      ...
      if (Number.isInteger(number) && number > 0) usedSerials.add(number);
    }

    let serial = 1;
    while (usedSerials.has(serial)) serial += 1;

    const studentId = `${prefix}${String(serial).padStart(4, "0")}`;
    const loginEmail = await generateStudentLoginIdentifier(studentId);
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (!user) throw new Error("Failed to create student user account.");

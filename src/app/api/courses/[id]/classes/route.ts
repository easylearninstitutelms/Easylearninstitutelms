...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const pe=requireRoles(session,ROLES);if(pe)return pe;await ensureCourseSchema();const {id}=await params;
...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
 const session=await getSession();if(!session?.instituteId)return NextResponse.json({error:"Unauthorized"},{status:401});
 const pe=requireRoles(session,ROLES);if(pe)return pe;await ensureCourseSchema();const {id}=await params;const b=await request.json();
 const title=String(b.title||"").trim();if(!title)return NextResponse.json({error:"Class title is required."},{status:400});
 const maxRows=rows(await db.execute(sql`SELECT COALESCE(MAX(class_no),0)::int AS "maxNo" FROM course_syllabus_classes WHERE course_id=${id} AND institute_id=${session.instituteId}`));
 const classNo=Number(b.classNo)||Number(maxRows[0]?.maxNo||0)+1;
 const saved=rows(await db.execute(sql`INSERT INTO course_syllabus_classes(institute_id,course_id,class_no,title,description,scheduled_date,start_time,end_time,status) SELECT ${session.instituteId},id,${classNo},${title},${b.description?String(b.description).trim():null},${b.scheduledDate||null},${b.startTime||null},${b.endTime||null},${b.status||"UPCOMING"} FROM courses WHERE id=${id} AND institute_id=${session.instituteId} RETURNING id,class_no AS "classNo",title,description,scheduled_date AS "scheduledDate",start_time AS "startTime",end_time AS "endTime",status`))[0];
 if(!saved)return NextResponse.json({error:"Course not found"},{status:404});return NextResponse.json({class:saved});
}

export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
 const session=await getSession();if(!session?.instituteId)return NextResponse.json({error:"Unauthorized"},{status:401});
 const pe=requireRoles(session,ROLES);if(pe)return pe;await ensureCourseSchema();const {id}=await params;const b=await request.json();const classId=String(b.classId||"");

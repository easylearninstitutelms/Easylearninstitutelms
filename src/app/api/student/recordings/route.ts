 ...
 try{
  ...
  const result=await db.execute(sql`
   SELECT r.id,'PROGRAMME' AS "recordingType",e.batch_id AS "batchId",r.syllabus_class_id AS "classId",
          ...
          COALESCE(b.name,'Direct Programme Enrollment') AS "batchName",p.name AS "programmeName",
          ps.semester_no AS "semesterNo",ps.name AS "semesterName",c.class_no AS "classNo",c.title AS "classTitle"
   ...
   WHERE r.institute_id=${session.instituteId}
     AND e.institute_id=${session.instituteId} AND e.status='ACTIVE'
     AND (e.programme_id=c.programme_id OR (b.status='ACTIVE' AND b.programme_id=c.programme_id AND b.semester_id=c.semester_id))
   UNION ALL
   SELECT r.id,'COURSE' AS "recordingType",e.batch_id AS "batchId",c.id AS "classId",
          r.title AS "recordingTitle",r.video_url AS "videoUrl",r.duration,r.created_at AS "createdAt",
          COALESCE(b.name,'Direct Course Enrollment') AS "batchName",co.name AS "programmeName",
   ...
   INNER JOIN enrollments e ON e.student_id=(SELECT id FROM students WHERE user_id=${session.userId} AND institute_id=${session.instituteId} LIMIT 1)
   LEFT JOIN batches b ON b.id=e.batch_id AND b.course_id=co.id
   WHERE r.institute_id=${session.instituteId}
     AND e.institute_id=${session.instituteId} AND e.status='ACTIVE'

...
export async function GET() {
  ...
  try {
    ...
    const rows = await db.execute(sql`
      SELECT e.id AS enrollment_id, e.batch_id AS batch_id, COALESCE(b.name,'Direct Programme Enrollment') AS batch_name,
        p.id AS programme_id, p.name AS programme_name, ps.id AS batch_semester_id,
        ...
        sc.status AS syllabus_status, 'PENDING' AS session_status,
        NULL AS taken_at, rec.id AS recording_id, rec.title AS recording_title,
      ...
      LEFT JOIN programme_syllabus_recordings rec ON rec.syllabus_class_id=sc.id

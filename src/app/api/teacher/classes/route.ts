...
export async function PATCH(request: Request) {
  try {
    ...
    /*
     * Class taken/completed status is still a teacher action.
     * Institute/Admin can add/edit programme syllabus and
     * recordings, but cannot mark a teacher's class as taken.
     */
    ...
    if (!classId || !batchId) {
      return NextResponse.json(
        {
          error: "Class and batch are required.",
        },
        { status: 400 }
      );
    }

    const allowed = rowsOf(
      await db.execute(sql`
        SELECT c.id

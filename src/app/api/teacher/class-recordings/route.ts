...
export async function POST(request: Request) {
  try {
    ...
    const allowed = rowsOf(await db.execute(sql\`
      SELECT sess.status AS "sessionStatus"
      FROM programme_syllabus_classes c
      INNER JOIN batches b ON b.programme_id = c.programme_id AND b.semester_id = c.semester_id
      INNER JOIN programme_syllabus_class_sessions sess ON sess.syllabus_class_id = c.id AND sess.batch_id = b.id AND sess.teacher_id = \${teacherId}
      WHERE c.id = \${classId} AND b.id = \${batchId} AND b.teacher_id = \${teacherId} AND b.institute_id = \${session.instituteId} AND b.status = 'ACTIVE'
      LIMIT 1
    \`));
    if (!allowed[0]) return NextResponse.json({ error: "This class is not assigned to you." }, { status: 403 });
    if (allowed[0].sessionStatus !== "COMPLETED") return NextResponse.json({ error: "Mark the class as taken before adding its recording." }, { status: 400 });
    const saved = rowsOf(await db.execute(sql\`
      ...
      VALUES (\${session.instituteId}, \${classId}, \${batchId}, \${teacherId}, \${title}, \${videoUrl}, \${duration})
...
export async function DELETE(request: Request) {
  try {
    const session = await getSession();
    if (!session?.instituteId || session.role !== "TEACHER") return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    await ensureAcademicSchema();
    const teacherId = await teacherIdOf(session);
    const body = await request.json();
    const classId = String(body.classId || "");
    const batchId = String(body.batchId || "");
    if (!teacherId || !classId || !batchId) return NextResponse.json({ error: "Class and batch are required." }, { status: 400 });
